...
//#include <drivers/nichelpers.h>
#include <drivers/pppwrap.h>
...
void ppp_putc(char c)
{
  ...
  #IF PPP_LCP_USE_ASYNC_MAP
      else if (c<0x20) {
         if (ppp_use_async_map) {
            if (ppp_async_map[c]) {ppp_putc_escape(c);} else {fputc(c,MODEM);}
         }
         else {
            ppp_putc_escape(c);
         }
...
#IF PPP_LCP_USE_PAP
   ppp_isp_has_pap=FALSE;
//   ppp_isp_ask_pap=TRUE;
#ENDIF

#IF PPP_LCP_USE_ASYNC_MAP
   for (i=0;i<32;i++) {
      ppp_async_map[i]=FALSE;
   }
   ppp_async_map[17]=TRUE;
   ppp_async_map[19]=TRUE;
   ppp_use_async_map=FALSE;      //will be set to true if we configure correctly
...
void debug_display_ppp_frame(PPP_OPTION_FRAME_HEADER *frame) {
   ...
   if (frame->protocol != PPP_COMP_PROT_IP) {
       debug(debug_putc,"CODE=%X  ID=%X  LEN=%LX",frame->code, frame->id, frame->len);
   }
}
...
void ppp_change_state(PPP_STATES new_state, int8 next_event) {
   debug(debug_putc,"\r\nNew State: %X (T=%U)", new_state, next_event);
   ppp_state=new_state;
   timer_set_s(next_event);
}

#IF PPP_LCP_USE_ASYNC_MAP
int32 ppp_get_map(void) {    //used to read async map from ppp header.  result is big endian
   int32 ret;
   int8 a,b,c,d;
   a=ppp_getc();
   b=ppp_getc();
   c=ppp_getc();
   d=ppp_getc();
//   ret=make32(a,b,c,d);  //THIS MUST BE WRONG?!?!
   ret=make32(d,c,b,a);
   return(ret);
}

void ppp_put_map(int32 val) { //puts result of ppp_map_to_int32 into ppp header.  since input is big endian, no conversion
   ppp_putc(make8(val,0));
   ppp_putc(make8(val,1));
   ppp_putc(make8(val,2));
   ppp_putc(make8(val,3));
}

int32 ppp_map_to_int32(void) {   //result is big endian.
   int8 i;
   int32 map=0;
   IP_ADDR x,n;
   for (i=31; i!=0xFF; i--) {
      shift_left(&map,4,(int1)ppp_async_map[i]);
   }
   x.Val=map;
   n.v[0]=x.v[3];
   n.v[1]=x.v[2];
...
void ppp_lcp_ipcp_recv_req(PPP_OPTION_FRAME_HEADER *frame) {
      ...
      */
      ...
      while (option_len) {
         ...
         else if ((sub_option_type == PPP_OPTION_LCP_AUTH)&&(sub_option_len==4)&&(frame->protocol==PPP_COMP_PROT_LCP)) {
            auth_method=ppp_getw();
            option_len-=2;
            if (auth_method != 0xC023) {  //PAP
               new_reject=1;
            }
            else if (!rejects) {
               ppp_isp_has_pap=TRUE;
//               ppp_isp_ask_pap=TRUE;
               ackd_lcp=TRUE;
        ...
        #IF PPP_LCP_USE_ASYNC_MAP
         ...
         if (new_reject) {//echo back rejected option, but do not go over our internal tx buffer size
            if (!rejects) {tx=0;}
            rejects++;
      ...
      if (!rejects) {
         ...
         if (ackd_lcp) {
           #IF PPP_LCP_USE_ASYNC_MAP
            if ((ppp_state == SERVER_ACKED_LCP)&&(!ppp_async_map_new))    //we acked him, he acked us, goto next stage
           #else
           ...
           #endif
           {
            ...
            else {   //need to wait until he acks us
      ...
      #IF PPP_LCP_USE_ASYNC_MAP
       if (finished_lcp) {
         ppp_use_async_map=ppp_attempt_async_map;  //start using async map now for txing messages
       }
      #ENDIF
}
...
void ppp_lcp_ipcp_recv(PPP_OPTION_FRAME_HEADER *frame) {
  ...
  //we dont send any IPCP requests that the ISP would reject, so dont worry about IPCP rejects
         ...
         while(option_len) {
            ...
            #IF PPP_LCP_USE_ASYNC_MAP
            if (sub_option_type == PPP_OPTION_LCP_ASYNC_MAP) {if (!ppp_async_map_new) ppp_attempt_async_map=FALSE;}
            #ENDIF
   ...
   /// *** ANSWER ACKS
      ...
      if (frame->protocol==PPP_COMP_PROT_LCP) { //find out which options he acked
         while(option_len) {
            ...
            #IF PPP_LCP_USE_ASYNC_MAP
            //if (sub_option_type == PPP_OPTION_LCP_ASYNC_MAP) {ppp_attempt_async_map=TRUE;}
            #ENDIF
         ...
         else {   //we need to ack their config message before we can go to next stage
            ppp_change_state(SERVER_ACKED_LCP,1);
         }
      }
  ...
  if (ppp_get_frame(&ppp_frame)) {
   ...
   if (timer_event() && connected_baudrate) {
      if ((ppp_state==PPP_STATE_INIT)||(ppp_state==CLIENT_ACKED_LCP)) {         //send out LCP config requests every 2 seconds until he acks us
        ...
        #IF PPP_LCP_USE_ASYNC_MAP
         if (ppp_attempt_async_map) {option_len+=6;}
        #ENDIF
        ...
        #IF PPP_LCP_USE_ASYNC_MAP
         if (ppp_attempt_async_map) {
            ppp_async_map_new=FALSE;
            ppp_putc(PPP_OPTION_LCP_ASYNC_MAP);
            ppp_putc(6);
            ppp_put_map(ppp_map_to_int32());
         }
        #ENDIF
        ...
        #ENDIF
         ...
         ppp_option_packet_begin(PPP_COMP_PROT_IPCP, PPP_CODE_REQ, ipcp_id++, 6);
