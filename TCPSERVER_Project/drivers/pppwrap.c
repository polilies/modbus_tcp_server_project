...
//returns current receive buffer identifier
//(in ppp case, anything other than BUFFER_INVALID should be ok)
int16 MACGetRxBuffer() {
   return(0);
}
...
void    MACPutHeader(MAC_ADDR *remote, int8 type, int16 dataLen) {
   ppp_tx_buff.rxpos = 0;
...
void    MACFlush(void) {
   if (ppp_tx_buff.txlen) {
      ...
      while (ppp_tx_buff.rxpos < ppp_tx_buff.txlen) {
         debug(debug_putc,"%X ",ppp_tx_buff.data[ppp_tx_buff.rxpos]);
...
//gets current tx buffer identifier
//(in ppp case, there is only one buffer so it doesnt matter what we return as long as its not INVALID_BUFFER)
int16 MACGetTxBuffer() {
   return(0);
}
