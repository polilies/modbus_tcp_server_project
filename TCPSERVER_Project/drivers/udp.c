...
UDP_SOCKET FindMatching_UDP_Socket(UDP_HEADER *h, NODE_INFO *remoteNode);
 ...
 *
 * Input:           localPort  - A valid port number.
 *
 *                  remoteNode - Remote Node info such as MAC and IP
 *                               address
                   ...
                   UDP_PORT remotePort)
{
    ...
    for ( s = 0; s < MAX_UDP_SOCKETS; s++ )
    {
        if ( p->localPort == INVALID_UDP_SOCKET )
        {
            ...
            if ( remoteNode )
            {
                //memcpy((char*)&p->remoteNode, remoteNode, sizeof(p->remoteNode));
                memcpy(&p->remoteNode, remoteNode, sizeof(NODE_INFO));
            }
...
void UDPFlush(void)
{
    ...
    //debug(debug_putc,"\r\n\r\nUDP FLUSH- AS:%U DS:%U SP:%LX DP:%LX",activeUDPSocket,DHCPSocket,p->localPort,p->remotePort);

    h.SourcePort        = swaps(p->localPort);
    h.DestinationPort   = swaps(p->remotePort);
    h.Length            = (int16)((int16)p->TxCount + (int16)sizeof(UDP_HEADER));
    ...
    IPPutArray(&h, sizeof(UDP_HEADER));
...
int1 UDPProcess(NODE_INFO *remoteNode, int16 len)
{
    ...
    s = FindMatching_UDP_Socket(&h, remoteNode);
...
UDP_SOCKET FindMatching_UDP_Socket(UDP_HEADER *h, NODE_INFO *remoteNode)
{
    ...
    for ( s = 0; s < MAX_UDP_SOCKETS; s++ )
    {
        ...
        {
            if ( ((p->remoteNode.IPAddr.Val == remoteNode->IPAddr.Val)||(p->remoteNode.IPAddr.Val == 0xFFFFFFFF))
                   && (p->remotePort == h->SourcePort) && (p->RxCount == 0) )
            {
                return s;
            }
            partialMatch = s;
    ...
    if ( partialMatch != INVALID_UDP_SOCKET )
    {
        ...
        //memcpy((char*)&p->remoteNode, remoteNode, sizeof(p->remoteNode) );
        memcpy(&p->remoteNode, remoteNode, sizeof(p->remoteNode) );
